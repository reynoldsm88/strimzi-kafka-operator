    ...
    private void init() {
        ...
        this.kafka = new KafkaBuilder()
                ...
                .withNewSpec()
                    .withNewKafka()
                        .withReplicas(3)
                        .withNewJbodStorageStorage()
                            .withVolumes(volumes)
                        .endJbodStorageStorage()
                    .endKafka()
                    .withNewZookeeper()
                        .withReplicas(1)
                    .endZookeeper()
                ...
                for (SingleVolumeStorage volume : this.volumes) {
                    if (volume instanceof PersistentClaimStorage) {
                        context.assertTrue(pvcs.stream().anyMatch(pvc -> {
                            String pvcName = AbstractModel.VOLUME_NAME + "-" + volume.getId() + "-"
                                    + KafkaCluster.kafkaPodName(NAME, podId);
                            boolean isDeleteClaim = ((PersistentClaimStorage) volume).isDeleteClaim();

                            boolean b = pvc.getMetadata().getName().equals(pvcName);
                            b &= Annotations.booleanAnnotation(pvc, AbstractModel.ANNO_STRIMZI_IO_DELETE_CLAIM,
